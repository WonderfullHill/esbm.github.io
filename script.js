...
function happyNewYear(direction,num){
	let i=0;
	let fireworks=[];
	...
	for(i=0;i<num;i++)
	{ 
		let shell = new Shell(shellFromConfig(shellSizeSelector())); 
		shell.shoot_index=i;
	...
	{ 
		setTimeout(()=>{
			...
			f.launch(f.shoot_index*mainStage.width/fireworks.length/mainStage.width,0.8);
		 
			if(i+1==fireworks.length)
			{  
				fireworks.splice(0,fireworks.length)
				
				setTimeout(()=>{
				happyNewYear("right",num+1);
...
function seqRandomFastShell() {
	...
	return 900 + Math.random() * 1600 + extraDelay;
}
